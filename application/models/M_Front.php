...
class M_Front extends CI_Model
{
    ...
    public function fetchdbabsen($kode_akun)
    {
        $today = $this->get_today_date;
        return $this->db->get_where('db_absensi', ['kode_akun' => $kode_akun, 'tgl_absen' => $today])->row_array();
    }
    ...
    public function do_absen()
    {
        ...
        if (strtotime($clocknow) >= strtotime($appsettings['absen_mulai']) && strtotime($clocknow) <= strtotime($appsettings['absen_mulai_to'])) {
            if ($this->db->get_where('db_absensi', ['tgl_absen' => $today, 'kode_akun' => $this->get_datasess['kode_akun']])->row_array()) {
                $data = [
                    'jam_masuk' => $clocknow
                ];
                $this->db->where('tgl_absen', $today)->where('kode_akun', $this->get_datasess['kode_akun']);
                $this->db->update('db_absensi', $data);
            } else {
                $data = [
                    'nama_pegawai' => $this->get_datasess['nama_lengkap'],
                    'kode_akun' => $this->get_datasess['kode_akun'],
                    'jam_masuk' => $clocknow,

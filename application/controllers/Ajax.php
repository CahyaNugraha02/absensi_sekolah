...
class Ajax extends CI_Controller
{
    ...
    public function absenajax()
    {
        ...
        $is_pulang = $this->db->get_where('db_absensi', ['tgl_absen' => $today, 'kode_akun' => $this->get_datasess['kode_akun']])->row_array();
        ...
        } elseif (strtotime($clocknow) >= strtotime($appsettings['absen_mulai_to']) && strtotime($clocknow) <= strtotime($appsettings['absen_pulang']) && $this->db->get_where('db_absensi', ['tgl_absen' => $today, 'kode_akun' => $this->get_datasess['kode_akun']])->row_array()) {
            $reponse = [
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
                'success' => false,
        ...
        if ($typesend == 'addpgw') {
            ...
            $validation = [
                ...
                [
                    ...
                    'label' => 'Umur Pegawai',
                    'rules' => 'required|xss_clean|max_length[2]|numeric',
                    'errors' => ['required' => 'You must provide a %s.', 'xss_clean' => 'Please check your form on %s.', 'numeric' => 'Karakter harus angka tidak boleh huruf pada %s.', 'max_length' => 'Angka umur terlalu panjang, Max Karakter 2!']
                ],
                [
        ...
        if ($dataabsen == 'datapgw') {
            ...
            foreach ($query->result() as $r) {
                $data[] = [
                    ...
                    $r->kelas,
                    $r->jenis_kelamin,
                    ($r->role_id == 1) ? '<span class="badge badge-danger ml-1">Administrator</span>' : (($r->role_id == 2) ? '<span class="badge badge-primary ml-1">Guru</span>' : (($r->role_id == 3) ? '<span class="badge badge-success ml-1">Siswa</span>' : '<span class="badge badge-secondary ml-1">Tidak Ada Role</span>')),
            ...
            $query = $this->db->get_where("db_absensi", ['kode_akun' => $datapegawai['kode_akun']]);
            foreach ($query->result() as $r) {
                $data[] = [
                    $no++,
                    $r->tgl_absen,
